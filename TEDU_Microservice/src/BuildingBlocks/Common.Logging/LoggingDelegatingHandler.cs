...
public class LoggingDelegatingHandler : DelegatingHandler
{
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("Sending request to {Url} - Method {Method} - Version", request.RequestUri, request.Method, request.Version);
        ...
        when(ex.InnerException is SocketException { SocketErrorCode: SocketError.ConnectionRefused })
        {
            ...
            _logger.LogCritical(ex, "Unable to connect to {Host}. Please check the configuration/settings to ensure the correct URL for the service has been configured", hostWithPort);
        }
